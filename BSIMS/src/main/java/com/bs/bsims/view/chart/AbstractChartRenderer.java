...
/**
 * Abstract renderer implementation, every chart renderer extends this class(although it is not
 * required it helps).
 */
public abstract class AbstractChartRenderer implements ChartRenderer {
    ...
    /**
     * If true maximum and current viewport will be calculated when chart data change or during data
     * animations.
     */
    ...
    protected void drawLabelTextAndBackground(Canvas canvas, char[] labelBuffer, int startIndex, int numChars, int autoBackgroundColor) {
        ...
        if (isValueLabelBackgroundEnabled) {
            ...
            // canvas.drawRect(labelBackgroundRect,
            // labelBackgroundPaint);//�������ˣ�������������ľ���
        ...
        // labelPaint.setARGB(255, 79, 159, 204);// ���ӵģ��ı�������ɫ
        labelPaint.setColor(autoBackgroundColor);
        String labelTitle = new String();
        for (int i = startIndex; i < labelBuffer.length; i++) {
            labelTitle += labelBuffer[i];
        }
        if (Float.parseFloat(labelTitle) > 9999) {
            labelTitle = CommonUtils.countNumberSecond(labelTitle);
        }
        canvas.drawText(labelTitle, textX, textY + 10, labelPaint);
