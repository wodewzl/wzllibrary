...
public class FloatUtils {
     ...
     * string begins at [endIndex] - [return value] and ends at [endIndex]. It's up to you to check
     * indexes correctness. Parameter [endIndex] can be helpful when you want to append some text to
     * formatted value.
     * 
    ...
    public static void computeAutoGeneratedAxisValues(float start, float stop, int steps, AxisAutoValues outValues) {
        ...
        steps = 5;
        int rawInterval = (int) range / steps;
        // double interval = roundToOneSignificantFigure(rawInterval);
        double interval = rawInterval * 1.0;
        ...
        double intervalValue = first;
        int valueIndex;
        int valuesNum = steps + 1;
        // for (intervalValue = first; intervalValue <= range; intervalValue += interval) {
        // ++valuesNum;
        // }
