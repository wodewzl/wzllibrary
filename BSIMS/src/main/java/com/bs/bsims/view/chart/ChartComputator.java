...
public class ChartComputator {
    ...
    /**
     * This rectangle represents the currently visible chart values ranges. The currently visible
     * chart X values are from this rectangle's left to its right. The currently visible chart Y
     * values are from this rectangle's top to its bottom.
     */
    ...
    /**
     * Warning! Viewport listener is disabled for all charts beside preview charts to avoid
     * additional method calls during animations.
     */
    ...
    /**
     * Sets the current viewport (defined by {@link #currentViewport}) to the given X and Y
     * positions.
     */
    ...
    /**
     * Translates chart value into raw pixel value. Returned value is absolute pixel X coordinate.
     * If this method return 0 that means left most pixel of the screen.
     */
    public float computeRawX(float valueX) {
        // TODO: (contentRectMinusAllMargins.width() / currentViewport.width()) can be recalculated
        // only when viewport
        // change.
     ...
     * pixel coordinates, if that pixel is within the chart region described by
     * {@link #contentRectMinusAllMargins}. If the point is found, the "dest" argument is set to the
     * point and this function returns true. Otherwise, this function returns false and "dest" is
     * unchanged.
     */
     ...
     * area is visible, this is simply the current size of {@link #contentRectMinusAllMargins}. If
    ...
    /**
     * Returns content rectangle with chart internal margins, for example for LineChart
     * contentRectMinusAxesMargins is bigger than contentRectMinusAllMargins by point radius, thanks
     * to that points are not cut on edges.
     * 
     * @see #setContentRect(int, int, int, int, int, int)
     */
    ...
    /**
     * Set current viewport to the same values as viewport passed in parameter. This method use deep
     * copy so parameter can be safely modified later. Current viewport must be equal or smaller
     * than maximum viewport.
     * 
     * @param viewport
     */
    ...
    /**
     * Set new values for curent viewport, that will change what part of chart is visible. Current
     * viewport must be equal or smaller than maximum viewport.
     */
    ...
    /**
     * Set maximum viewport to the same values as viewport passed in parameter. This method use deep
     * copy so parameter can be safely modified later.
     * 
     * @param maxViewport
     */
