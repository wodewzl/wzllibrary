...
public class AxesRenderer {
    private static final int DEFAULT_AXIS_MARGIN_DP = 6;
     ...
     * array are used to measure text width.
     */
    ...
    private Paint[] labelPaintTab = new Paint[] {
            new Paint(), new Paint(), new Paint(), new Paint()
    };
    private Paint[] namePaintTab = new Paint[] {
            new Paint(), new Paint(), new Paint(), new Paint()
    };
    private Paint[] linePaintTab = new Paint[] {
            new Paint(), new Paint(), new Paint(), new Paint()
    };
    ...
    private FontMetricsInt[] fontMetricsTab = new FontMetricsInt[] {
            new FontMetricsInt(), new FontMetricsInt(), new FontMetricsInt(), new FontMetricsInt()
    };
    /**
     ...
     * auto-generated values should be drawn to avoid overdrawing. Used only for auto axes.
     */
     ...
     */
    private AxisAutoValues[] autoValuesBufferTab = new AxisAutoValues[] {
            new AxisAutoValues(), new AxisAutoValues(), new AxisAutoValues(), new AxisAutoValues()
    };
    ...
    private void prepareAutoGeneratedAxis(Axis axis, int position) {
        ...
        FloatUtils.computeAutoGeneratedAxisValues(start, stop, contentRectDimension / labelDimensionForStepsTab[position] / 2, autoValuesBufferTab[position]);
        // Reinitialize tab to hold lines coordinates.
    ...
    private void drawAxisLabelsAndName(Canvas canvas, Axis axis, int position) {
        ...
        for (int valueToDrawIndex = 0; valueToDrawIndex < valuesToDrawNumTab[position]; ++valueToDrawIndex) {
            ...
            if (axis.hasTiltedLabels()) {
                ...
                int count = 1;
                if (count < (charsNumber - 2)) {
                    count++;
                    String labelTitle = new String();
                    for (int i = labelBuffer.length - charsNumber; i < labelBuffer.length; i++) {
                        labelTitle += labelBuffer[i];
                    }
                    if (Float.parseFloat(labelTitle) > 9999) {
                        labelTitle = CommonUtils.countNumberFive(labelTitle);
                        canvas.drawText(labelTitle, labelX + labelX / 4, labelY, labelPaintTab[position]);

                    }
                    else {
                        canvas.drawText(labelTitle, labelX, labelY, labelPaintTab[position]);
                    }
