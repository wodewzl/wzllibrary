...
public class PieChartView extends View implements Runnable {
    ...
    boolean falge = true;
    // 默认颜色6A5ACD
    private static final String[] DEFAULT_ITEMS_COLORS = {
            "#00A1E9", "#F9B300", "#786AAF", "#009FC9", "#DFB734", "#7E83BF",
            "#009FA7", "#C5B754", "#8599CD", "#009D85", "#A5B36C", "#8DAFDB",
            "#009B62", "#81AF83", "#95C5E9", "#009946", "#56A795", "#9FD9F7",
            "#00A8FF", "#39C0F0", "#76CBF4", "#FFB310", "#37A532", "#CD853F", "#006400", "#FF4500", "#D8BFD8", "#4876FF", "#FF00FF", "#FF83FA",
            "#0000FF", "#363636", "#FFDAB9", "#90EE90", "#8B008B", "#00BFFF", "#00FF00", "#006400", "#00FFFF", "#00FFFF", "#668B8B", "#000080", "#008B8B"
    };
    ...
    private static final int DEFAULT_RADIUS = 50;// 默认饼状图半径，不包括边缘
    ...
    private Boolean flagboolean = false;// 用于中心圆里面内容换行
     ...
     * 
     * @param context 上下文
     * @param itemColors 饼状图上从第一个块开始，各个块的颜色； 如两块：传值String[] itemColors = {"#000000", "#FFFFFF"}，
     *            为空，则显示随机颜色。参见： {@link #setItemsColors}
     * @param itemSizes 饼状图上各个块的大小；如两块：传值float[] itemSizes = {(float) 50.0, (float)
     *            50.0}，为空，则只显示圆环。参见： {@link #setItemsSizes}
     * @param total 饼状图所表示的整体大小；大于或等于 itemSizes 各项数据之和，若小于，则等于，默认等于。参见： {@link #setTotal}
     * @param radius 饼状图的半径，但不包括边缘圆环的粗度。默认为 100。参见：{@link #setRaduis}
     * @param strokeWidth 饼状图边缘圆环的粗度；默认为 3。参见：{@link #setStrokeWidth}
     * @param strokeColor 饼状图边缘圆环的颜色，默认为黑色(#000000).参见：{@link #setStrokeColor}
     * @param rotateWhere 若动画已开启，则饼状图上被点击的块旋转到的位置；左侧：{@link #TO_LEFT}； 顶部： {@link #TO_TOP}； 右侧：
     *            {@link #TO_RIGHT}； 底部： {@link #TO_BOTTOM} ； 不旋转：{@link #NO_ROTATE}； 默认为 ：
     *            {@link #TO_RIGHT}； 参见： {@link #setRotateWhere}
     * @param separateDistence 被选中的块的偏移距离；为了凸显备选中的块，偏移凸显；若为负或未设置，默认为 ：
     *            {@link #DEFAULT_SEPARATE_DISTENCE}。 参见： {@link #setSeparateDistence};
     * @param rotateSpeed 若动画开启，则表示旋转动画的速度；若为负或未设置，默认为 ：{@link #DEFAULT_ANIM_SPEED}。 参见：
     *            {@link #setRotateSpeed};
     */
    public PieChartView(Context context, String[] itemColors, float[] itemSizes, float total, int radius, int strokeWidth, String strokeColor, int rotateWhere, float separateDistence,
            float rotateSpeed) {
        super(context);
     ...
     * 
     * @param items 各个块的大小，由 float 类型数据组成的数组
     */
     ...
     * @param total
     */
    public void setActualTotal(float total) {
        this.totalNum = total;
    }
     ...
     * 
     * @param total 大于或等于 itemSizes 各项数据之和，若小于，则等于，默认等于。
     */
    ...
    public void relaseTotal(float total) {
        this.total = total;
    }
     ...
     * 
     * @param rotateSpeed 数值在0.5~5.0之间，小于0.5则为0.5；大于5.0，则为5.0。若为负或未设置，默认为 ：
     ...
     * 设置要显示的item
     * 
     * @param position 被选中的块的 id；即描述各个块的大小的数组中某项数据的id号。
     * @param anim 是否开启动旋转画；true为开启
     * @param listen 是否可以监听到此消息；true为可以监听；监听方法参见：
     ...
     * 设置饼状图上被点击的块转到的位置
     * 
     * @param rotateWhere <br/>
     *            位置： <li>左侧：{@link #TO_LEFT}</li> <li>顶部：{@link #TO_TOP}</li> <li>右侧：
     *            {@link #TO_RIGHT}</li> <li>底部：{@link #TO_BOTTOM}</li> <br/>
     ...
     * 判断饼状图上被点击的块旋转到的位置
     * 
     * @return 位置： <li>左侧：{@link #TO_LEFT}</li> <li>顶部：{@link #TO_TOP}</li> <li>右侧：{@link #TO_RIGHT}
     *         </li> <li>底部：{@link #TO_BOTTOM}</li><li>不旋转： {@link #NO_ROTATE}；</li> <br/>
     */
     ...
     * 设置被选中的块分离的距离
     * 
     * @param separateDistence 若为负或未设置，默认为 ：{@link #DEFAULT_SEPARATE_DISTENCE}
     */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (itemsAngle != null && itemsBeginAngle != null) {
            ...
            for (int i = 0; i < itemsAngle.length; i++) {
                ...
                if (itemsColors[i].equals("#BAC5D1") && itemsAngle.length > 1) {
                    itemsColors[i] = "#4D9E2E";
                    paint.setColor(Color.parseColor("#4D9E2E"));
                } else {
            ...
            canvas.drawCircle(centerX, centerY, radius / 2 + context.getResources().getDimension(R.dimen.statistics_circle_radius), p1);
            p1.setColor(Color.WHITE);
            canvas.drawCircle(centerX, centerY, radius / 2, p1);
            // drawImg(canvas, centerX, centerY, radius / 2);
            // drawText(canvas, centerX, centerY - radius / 5, (int) radius /
            // 2);
            drawText(canvas, radius, radius, (int) radius / 2);
            // drawText01(canvas, radius, radius, (int) radius / 2);
        }
    ...
    public void setShowInfo(String showInfo, Boolean bool) {
        this.showInfo = showInfo;
    ...
    private void drawText(Canvas canvas, float x, float y, int radius) {
        ...
        if (flagboolean) {
            // 用于内容换行
            String type = showInfo.split(",")[0];
            String content = showInfo.split(",")[1];
            canvas.drawText(type, x - textPaint.measureText(type) / 2, y, textPaint);
            textPaint.setColor(Color.BLACK);
            textPaint.measureText(content);
            canvas.drawText(content, x - textPaint.measureText(content) / 2, y + 60, textPaint);
        } else {
            textPaint.setColor(Color.BLACK);
            textPaint.measureText(showInfo);
            canvas.drawText(showInfo, x - textPaint.measureText(showInfo) / 2, y + 40, textPaint);
        }
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if ((x1 - r) * (x1 - r) + (y1 - r) * (y1 - r) - r * r <= 0) {
                        ...
                        if (bitmap != null && triggerX <= x1 && x1 <= triggerX + bitmap.getWidth() && triggerY <= y1 && y1 <= triggerY + bitmap.getHeight()) {
                            notifyTriggerClickListeners();
                        } else if ((getWidth() / 2 - x1) * (getWidth() / 2 - x1) + (getHeight() / 2 - y1) * (getHeight() / 2 - y1) - (radius / 2) * (radius / 2) <= 0) {
                            // 点击了圆内空白区域也触发
                            notifyTriggerClickListeners();
                        } else {
                            // 在点击了扇形
    ...
    private void setDefaultColor() {
        ...
        /**
         * 由于饼子图颜色会重复，所以当时if判断有 if (itemsSizes != null && itemsSizes.length > 0&&itemsColors==null)
         **/
        if (itemsSizes != null && itemsSizes.length > 0) {
            ...
            if (itemsColors.length > 1) {
                for (int i = 0; i < itemsColors.length; i++) {
                    if (itemsColors[i].equals("#BAC5D1")) {
                        itemsColors[i] = "#4D9E2E";
                        break;
                    }
                }
     ...
     * 添加选中饼状图上某块的消息监听
     * 
     * @param listener 监听器；参见：{@link OnPieChartItemSelectedLinstener}
     */
