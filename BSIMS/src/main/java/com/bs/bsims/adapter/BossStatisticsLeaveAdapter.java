...
public class BossStatisticsLeaveAdapter extends BSBaseAdapter<BossStatisticsAttendanceVO> {
    private Context mContext;
    private String mMode = "1";

    public BossStatisticsLeaveAdapter(Context context) {
        super(context);
        mContext = context;
    }
    ...
    public void updateData(List<BossStatisticsAttendanceVO> list, int sort, String mode) {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (!list.get(i).isVisible())
                continue;
            BossStatisticsAttendanceVO vo = list.get(i).getList().get(sort);
            vo.setDname(list.get(i).getName());
            sortList.add(vo);
        }
        sortList(sortList);
