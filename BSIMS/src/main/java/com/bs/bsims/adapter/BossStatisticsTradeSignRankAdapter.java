...
public class BossStatisticsTradeSignRankAdapter extends BSBaseAdapter<BossStatisticsTradeSignVo> {
    private Context mContext;
    private String mCuurentTime = "";
    private String mPreviousTime = "";
    private float mMaxCount = 1;

    public BossStatisticsTradeSignRankAdapter(Context context) {
        super(context);
        mContext = context;
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        BossStatisticsTradeSignVo vo = mList.get(position);
        ...
        holder.mCountTv.setText(vo.getSignCountShow());

        holder.mCountPb.setProgress((int) ((Float.parseFloat(CommonUtils.isNormalData(vo.getSignCount())) / mMaxCount) * 100));
    ...
    @Override
    public void updateData(List<BossStatisticsTradeSignVo> list) {
        if (list == null) {
            super.updateData(list);
            return;
        }
        for (int i = 0; i < list.size(); i++) {
            if (Float.parseFloat(CommonUtils.isNormalData(list.get(i).getSignCount())) > mMaxCount) {
                mMaxCount = Float.parseFloat(CommonUtils.isNormalData(list.get(i).getSignCount()));
            }
        }
