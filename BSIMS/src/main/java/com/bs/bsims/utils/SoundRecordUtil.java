...
import com.bs.bsims.view.BSFloatView;
...
public class SoundRecordUtil implements OnClickListener {
    ...
    private int x, y;
    private int mStatus = 0; // 1为客户点击按钮悬浮 其它为录音
    ...
    public SoundRecordUtil(Context context, RecordResult recordResult, int x, int y, int status) {
        this.mContext = context;
        this.mRecordResult = recordResult;
        this.x = x;
        this.y = y;
...
//        SpeechUtility.createUtility(mContext, SpeechConstant.APPID + "=555942de");
    ...
    private void createView() {
        ...
        mBSFloatView.setId(123);
        mBSFloatView.setOnClickListener(this);
        // mBSFloatView.setImageResource(R.drawable.record); // 这里简单的用自带的icon来做演示

        if (mStatus == 1) {
            mBSFloatView.setBackgroundResource(R.drawable.crm_add);
        } else {
            mBSFloatView.setBackgroundResource(R.drawable.record);
        }
