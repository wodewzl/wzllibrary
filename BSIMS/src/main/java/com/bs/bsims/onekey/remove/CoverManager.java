...
public class CoverManager {
    ...
    public void finish(final View target, final float x, final float y) {
        /**
         * if click very quick. Drop may be finished before first frame. It will cause SurfaceView
         * render error.
         */
    ...
    // status 1为不移动就消除
    public void finish(final View target, final float x, final float y, final int status) {
        /**
         * if click very quick. Drop may be finished before first frame. It will cause SurfaceView
         * render error.
         */
        target.postDelayed(new Runnable() {
            ...
            @Override
            public void run() {
                mDropCover.finish(target, x, y, status);
                mDropCover.setOnDragCompeteListener(null);
            }
        }, 30);
    ...
    private Bitmap drawViewToBitmap(View view) {
        ...
        int width = CommonUtils.getViewWidth(view);
        int height = CommonUtils.getViewHigh(view);
    ...
    /**
     * please call it before animation start Notice: the unit is frame.
     * 
     * @param maxDistance
     */
