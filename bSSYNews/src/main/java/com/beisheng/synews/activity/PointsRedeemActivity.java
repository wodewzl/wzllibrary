...
import com.beisheng.synews.adapter.PointsRedeemAdapter;
...
public class PointsRedeemActivity extends BaseActivity implements OnRefreshListener, LoadMoreListener {
    private PointsMallVO mPointsMallVO;
    private BSListViewLoadMore mListView;
    private PointsRedeemAdapter mAdapter;
    ...
    public void initView() {
        ...
        mAdapter = new PointsRedeemAdapter(this);
    ...
    public boolean getData() {
        try {
            ...
            if (hasNetWork()) {
                String jsonStr = HttpClientUtil.getRequest(this, Constant.DOMAIN_NAME + Constant.REDEEM_RECORDS_URL, map);
                mPointsMallVO = gson.fromJson(jsonStr, PointsMallVO.class);
                saveJsonCache(Constant.REDEEM_RECORDS_URL, map, jsonStr);

            } else {
                String oldStr = getCacheFromDatabase(Constant.REDEEM_RECORDS_URL, map);
