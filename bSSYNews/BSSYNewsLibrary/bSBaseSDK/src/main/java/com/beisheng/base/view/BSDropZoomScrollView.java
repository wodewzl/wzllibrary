...
@SuppressLint("NewApi")
public class BSDropZoomScrollView extends ScrollView implements View.OnTouchListener {
    // 记录首次按下位置
    ...
    public BSDropZoomScrollView(Context context) {
        super(context);
    }

    public BSDropZoomScrollView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public BSDropZoomScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        if (getChildAt(0) != null) {
            ...
            if (vg.getChildAt(0) != null) {
                if (vg.getChildAt(0) instanceof ImageView) {
                    // 头部的图片而不是View的背景被拉伸
                    dropZoomView = vg.getChildAt(0);
                    setOnTouchListener(this);
                } else {
                    ViewGroup vgg = (ViewGroup) vg.getChildAt(0);
                    if (vgg.getChildAt(0) != null) {
                        // 头部的背景被拉伸
                        dropZoomView = vgg.getChildAt(0);
                        setOnTouchListener(this);
                    }
                }
            }
