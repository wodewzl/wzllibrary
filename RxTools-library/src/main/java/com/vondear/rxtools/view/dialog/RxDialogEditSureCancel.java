...
public class RxDialogEditSureCancel extends RxDialog {
    ...
    private int mType=1;//1为一个输入框2为2个输入框
    ...
    private void initView() {
        ...
        mEt02= (EditText) dialog_view.findViewById(R.id.et_02);
        if(mType==1){
            mEt02.setVisibility(View.GONE);
        }else{
            mEt02.setVisibility(View.VISIBLE);
        }
    ...
    public RxDialogEditSureCancel(Activity context,int type) {
        super(context);
        mType=type;
        initView();
    }
